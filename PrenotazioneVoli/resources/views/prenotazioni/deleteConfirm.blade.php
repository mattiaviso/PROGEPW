    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb justify-content-end">
            ...
            <li class="breadcrumb-item active" aria-current="page">
                {{trans('messages.cancellaPasseggeroPrenotazione')}}
            </li>
        </ol>
    </nav>
</div>
...
<div class="container mt-5 mb-5">
    <div class="row justify-content-center mt-5 mb-5">
        <div class="col-md-6 text-center mt-5 mb-5">
            <h3>{{trans('messages.sureEliminare')}} {{$passeggero->nome}} {{$passeggero->cognome}} {{trans('messages.dallaPrenotazione')}}
            </h3>
            <p>{{trans('messages.unaVoltaEseguita')}}</p>

            <form name="volo" method="post"
                action="{{ route('passeggeri.elimina', ['prenotazione' => $prenotazione->id, 'passeggero' => $passeggero->id]) }}">
                @method('DELETE')
                @csrf
